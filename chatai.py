...
from llama31B import chat as llama_chat
from phi import chat_phi
import threading
...
def llama(user_input, results):
    result_llama = llama_chat(user_input)
    results["llama31"] = result_llama

# Function to call PHI model
def phimodal(user_input, results):
    result_phi = chat_phi(user_input)
    results["phimodal"] = result_phi

# Function to call Azure OpenAI (GPT-4)
def gpt(userinput, results):
    try:
...
def chat_with_models(user_input):
    results = {}
    threads = [
        threading.Thread(target=gpt, args=(user_input, results)),
        threading.Thread(target=phimodal, args=(user_input, results)),
        threading.Thread(target=llama, args=(user_input, results)),
    ]
